...
Applicable to ALL datasets:
  ...
  4. TemporalRangeCompliance       - Fraction of records inside Omega_real
                                      and correctly anchored for Relative time
...
def _title_label(value: str) -> str:
    return value.strip().replace("_", " ").title().replace(" ", "")


def detect_periodicity(
    intervals: np.ndarray,
    periodicity_override: Optional[str] = None,
) -> Tuple[str, Optional[float]]:
    """
    Classify a series as Regular or Irregular.

    If a periodicity value is supplied from ``data/configs/<dataset>.json``,
    that value is used directly. Otherwise, falls back to the historical
    interval coefficient-of-variation heuristic.

    Parameters
    ----------
    intervals : np.ndarray
        Array of consecutive time differences across all trajectories.
    periodicity_override : str, optional
        Configured periodicity (``"regular"`` or ``"irregular"``).
    """
    if periodicity_override is not None:
        periodicity = _title_label(periodicity_override)
        representative_interval = None
        if periodicity == "Regular" and len(intervals) > 0:
            median_interval = float(np.median(intervals))
            representative_interval = median_interval if median_interval > 0 else None
        return periodicity, representative_interval
...
def detect_time_representation(
    ...
    key_col: str,
    time_col: str,
    time_representation_override: Optional[str] = None,
) -> str:
    """
    Detect whether timestamps are Absolute (calendar) or Relative (elapsed).

    If a time representation value is supplied from
    ``data/configs/<dataset>.json``, that value is used directly. Otherwise,
    falls back to the historical first-timestamp heuristic.
    ...
    time_col : str
        Timestamp column.
    time_representation_override : str, optional
        Configured time representation (``"absolute"`` or ``"relative"``).
    """
    if time_representation_override is not None:
        return _title_label(time_representation_override)
...
def metric_temporal_range_compliance(
    ...
    Absolute-time series use the observed real timestamp range as
    Omega_real = [T_start, T_end]. Relative-time series use the elapsed-time
    domain Omega_real = [0, max_k D_k^real], where D_k^real is the duration
    of real trajectory k. For Relative time, the first observation of each
    synthetic trajectory must also be anchored at elapsed time zero.

    Returns the aggregate compliance score together with diagnostic range and
    origin violation rates.
    ...
    """
    is_relative = time_repr == "Relative"
    if is_relative and pd.api.types.is_datetime64_any_dtype(synth_df[time_col]):
        raise ValueError(
    ...
    synth_eval = synth_df.reset_index(drop=True)
    synth_times_all = to_numeric_time(synth_eval[time_col])
    n_synth = len(synth_times_all)
    if n_synth == 0:
        return {
            "TemporalRangeCompliance": 1.0,
        }

    if is_relative:
        real_durations = []
        for _, group in real_df.groupby(key_col, sort=False):
            times = to_numeric_time(group.sort_values(time_col)[time_col])
            times = times[np.isfinite(times)]
            if len(times) >= 1:
                real_durations.append(times[-1] - times[0])
        if not real_durations:
            return {
                "TemporalRangeCompliance": 0.0,
            }
        omega_low = 0.0
        omega_high = float(np.nanmax(real_durations))
    else:
        real_times_all = to_numeric_time(real_df[time_col])
        real_times_all = real_times_all[np.isfinite(real_times_all)]
        if len(real_times_all) == 0:
            return {
                "TemporalRangeCompliance": 0.0,
            }
    ...
    range_violations = (
        ~np.isfinite(synth_times_all)
        | (synth_times_all < omega_low)
        | (synth_times_all > omega_high)
    )
    combined_violations = range_violations.copy()

    if is_relative:
        for _, group in synth_eval.groupby(key_col, sort=False):
            if len(group) == 0:
                continue
            first_idx = group.sort_values(time_col).index[0]
            first_time = to_numeric_time(synth_eval.loc[[first_idx], time_col])[0]
            if not np.isfinite(first_time) or not np.isclose(first_time, 0.0):
                combined_violations[first_idx] = True

    compliance = 1.0 - float(np.sum(combined_violations) / n_synth)

    return {
        "TemporalRangeCompliance": compliance,
    }
...
def compute_data_characteristics(
    ...
    time_repr_override : str, optional
        ...
        ``"Relative"``.
    """
    periodicity, representative_interval = detect_periodicity(
        real_intervals,
        periodicity_override=periodicity,
    )

    if time_repr_override is not None:
        time_repr = detect_time_representation(
            real_df,
            key_col,
            time_col,
            time_representation_override=time_repr_override,
        )
...
def main() -> None:
    ...
    temporal_range_details = metric_temporal_range_compliance(
        real_df, synth_df, args.key_col, args.time_col, time_repr
    )
    metrics.update(temporal_range_details)
