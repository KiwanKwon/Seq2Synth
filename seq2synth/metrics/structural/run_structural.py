...
def _time_col(cfg: Any, table_name: str) -> str | None:
    configured = (getattr(cfg, "table_time_cols", None) or {}).get(table_name)
    if configured:
        return configured
    datetime_cols = _datetime_cols(cfg.tables[table_name])
...
def _infer_time_bin(
    ...
    if table_name not in real_bundle or time_col not in real_bundle[table_name].columns:
        return "1D"
...
def _schema_complexity_kinds(cfg: Any) -> set[str]:
    configured = getattr(cfg, "data_structure_kinds", set())
    if configured:
        return configured
    ...
    if any(len(children) > 1 for children in children_by_parent.values()):
        inferred.add("multi-child")
    if any(count > 1 for count in parent_counts.values()):
        inferred.add("multi-parent")
    if not inferred:
        inferred.add("linear" if cfg.relationships else "single")
    return inferred
...
def _build_metric_specs(cfg: Any, real_bundle: dict[str, pd.DataFrame]) -> list[MetricSpec]:
    ...
    schema_kinds = _schema_complexity_kinds(cfg)
    has_multi_child = "multi-child" in schema_kinds
    has_multi_parent = "multi-parent" in schema_kinds
    ...
    if has_multi_child or has_multi_parent:
        for rel in cfg.relationships:
            parent = rel.get("parent_table_name")
            child = rel.get("child_table_name")
            ...
            if parent not in cfg.tables or child not in cfg.tables or not fk:
                continue
            child_time_col = _time_col(cfg, child)
            if has_multi_child and child_time_col:
                time_bin = _infer_time_bin(real_bundle, child, child_time_col)
                specs.append(
                    MetricSpec(
            ...
            if has_multi_child:
                ...
                for parent_col in parent_nums:
                    for child_col in child_nums:
                        if child_time_col:
                            time_bin = _infer_time_bin(
                                real_bundle, child, child_time_col
                            )
                            metric_name = (
                                f"dynamic_khop__{parent}__{parent_col}"
                                f"__{child}__{child_col}"
                            ).replace(" ", "_")
                            specs.append(
                                MetricSpec(
                                    name=metric_name,
...
def _run_variant(
    ...
    payload = {
        ...
        "metadata": {
            ...
            "data_structure_kinds": sorted(_schema_complexity_kinds(cfg)),
            "table_time_cols": getattr(cfg, "table_time_cols", None) or {},
