...
def transition_matrix_tvd_complement(real_df: pd.DataFrame, synth_df: pd.DataFrame,
    ...
    real_time_grid = pd.Index(sorted(real_df[time_col].dropna().unique()))
    valid_next_time = {
        real_time_grid[i]: real_time_grid[i + 1]
        for i in range(len(real_time_grid) - 1)
    }
    if not valid_next_time:
        return np.nan
    ...
    def build_transition_probs(df: pd.DataFrame) -> tuple[pd.Series, pd.Index]:
        ...
        next_time = work.groupby(key_col, sort=False)[time_col].shift(-1)
        transitions = pd.DataFrame(
            {
                "time": work[time_col].to_numpy(),
                "next_time": next_time.to_numpy(),
                "from": work[cat_col].to_numpy(),
                "to": next_cat.to_numpy(),
            }
        ).dropna(subset=["time", "next_time", "from", "to"])
        expected_next_time = transitions["time"].map(valid_next_time)
        transitions = transitions[
            expected_next_time.notna()
            & transitions["next_time"].eq(expected_next_time)
        ]
...
def autocorrelation_diagnostics(
    ...
    """Return lightweight checks explaining AutoCorrelation computability."""
    ...
    if flow_independence == "independent":
        def flow_stats(df: pd.DataFrame) -> dict:
            lengths = (
                df[[key_col, num_col]]
                .dropna(subset=[key_col, num_col])
                .groupby(key_col, dropna=False)
                .size()
            )
            valid = lengths[lengths >= min_required]
            return {
                "flows": int(len(lengths)),
                "valid_flows": int(len(valid)),
                "median_sequence_length": (
                    float(lengths.median()) if not lengths.empty else np.nan
                ),
                "max_sequence_length": (
                    int(lengths.max()) if not lengths.empty else 0
                ),
            }

        real_stats = flow_stats(real_df)
        synth_stats = flow_stats(synth_df)
        can_compute = real_stats["valid_flows"] > 0 and synth_stats["valid_flows"] > 0
        reason = (
            "ok"
            if can_compute
            else f"need at least one flow per side with length >= {min_required}"
        )
        return {
            "mode": flow_independence,
            "max_lag": max_lag,
            "min_required_sequence_length": min_required,
    ...
    def macro_stats(df: pd.DataFrame) -> dict:
        macro = (
            df[[time_col, num_col]]
            .dropna(subset=[time_col, num_col])
            .groupby(time_col)[num_col]
            .mean()
            .sort_index()
        )
        return {
            "time_points": int(len(macro)),
            "min_required_time_points": min_required,
        }

    real_stats = macro_stats(real_df)
    synth_stats = macro_stats(synth_df)
    can_compute = (
        real_stats["time_points"] >= min_required
        and synth_stats["time_points"] >= min_required
    )
    reason = (
        "ok"
        if can_compute
        else f"need at least {min_required} time points per side"
    )
    return {
        "mode": flow_independence,
        "max_lag": max_lag,
...
def coerce_panel_time_values(series: pd.Series) -> pd.Series:
    if pd.api.types.is_datetime64_any_dtype(series) or pd.api.types.is_numeric_dtype(series):
        return series

    text = series.astype("string").str.strip()
    valid_text = text.dropna()
    if not valid_text.empty and valid_text.str.fullmatch(r"\d{1,2}-\d{1,2}").mean() >= 0.8:
        converted = pd.to_datetime(text, format="%m-%d", errors="coerce")
...
def transform_to_3d_array(
    ...
    has_duplicate_time_keys = work.duplicated(subset=[id_col, time_col]).any()
    if has_duplicate_time_keys:
        work["_seq2synth_input_order"] = np.arange(len(work))
        work[time_col] = coerce_panel_time_values(work[time_col])
        work = (
            work.sort_values(
                [id_col, time_col, "_seq2synth_input_order"],
                kind="mergesort",
                na_position="last",
            )
            .drop_duplicates(subset=[id_col, time_col], keep="first")
            .drop(columns="_seq2synth_input_order")
        )
    else:
    ...
    if fill_method not in (None, "none"):
        ...
        for current_id in id_index:
            ...
            if fill_method == "edge_nearest":
                group = group.ffill().bfill() if has_duplicate_time_keys else edge_fill_group(group)
...
def compute_all_longitudinal_metrics(
    ...
    for col in num_cols:
        print(f"  Processing: {col}")
        ac_diagnostics[col] = autocorrelation_diagnostics(
            real_df, synth_df, key_col, time_col, col,
            flow_independence, max_lag
        )
        diag = ac_diagnostics[col]
        if flow_independence == "independent":
            print(
                "    AutoCorrelation check: "
                f"min_len={diag['min_required_sequence_length']}, "
                f"real_valid_flows={diag['real']['valid_flows']}/{diag['real']['flows']}, "
                f"synth_valid_flows={diag['synth']['valid_flows']}/{diag['synth']['flows']}, "
                f"can_compute={diag['can_compute']}"
            )
        else:
            print(
                "    AutoCorrelation check: "
                f"min_time_points={diag['min_required_time_points']}, "
                f"real_time_points={diag['real']['time_points']}, "
                f"synth_time_points={diag['synth']['time_points']}, "
                f"can_compute={diag['can_compute']}"
            )
    ...
    results['metadata']['AutoCorrelationSimilarity_diagnostics'] = ac_diagnostics
