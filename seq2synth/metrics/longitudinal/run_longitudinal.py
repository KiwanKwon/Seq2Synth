...
def _longitudinal_tables(cfg) -> list[str]:
    configured = getattr(cfg, "table_time_cols", None) or {}
    return [table_name for table_name in cfg.tables if table_name in configured]
...
def _prepare_frames(cfg, model_dir: Path, variant: str, table_name: str):
    if table_name not in cfg.tables:
        raise ValueError(f"No table found in metadata for {cfg.name}/{table_name}")
    key_col = _infer_key_col(cfg, table_name)
    time_col = _infer_time_col(cfg, table_name)
    if key_col is None:
        raise ValueError(f"Could not infer key column for {cfg.name}/{table_name}")
    if time_col is None:
        raise ValueError(f"Could not infer time column for {cfg.name}/{table_name}")

    real_df = flatten_table(cfg, table_name, cfg.real_dir)
    synth_df = flatten_table(cfg, table_name, cfg.real_dir, model_dir, variant)
    real_df = _apply_longitudinal_time(real_df, cfg.name, key_col, time_col)
    synth_df = _apply_longitudinal_time(synth_df, cfg.name, key_col, time_col)
    real_df = _coerce_time(real_df, time_col)
    synth_df = _coerce_time(synth_df, time_col)

    num_cols, cat_cols = _feature_columns(cfg, _ancestor_tables(cfg, table_name), key_col, time_col)
    ...
    return table_name, key_col, time_col, real_df, synth_df, num_cols, cat_cols
...
def _compute_default_max_lag(df: pd.DataFrame, key_col: str) -> int:
    sequence_lengths = df.groupby(key_col, dropna=True).size()
    if sequence_lengths.empty:
        return 0
    median_length = int(sequence_lengths.median())
    eighth_lag = median_length // 8
    return max(1, eighth_lag)
...
def _combine_table_results(
    table_results: dict[str, dict[str, Any]],
    *,
    dataset: str,
    model: str,
    variant: str,
    requested_max_lag: int | None,
    skip_tt_wasserstein: bool,
    compute_tt_standalone: bool,
    tt_num_iter_max: int,
    tt_max_flows: int | None,
    tt_sample_seed: int,
) -> dict[str, Any]:
    metric_names = (
    ...
    summary = {
        metric: _safe_mean(
            [result.get("summary", {}).get(metric) for result in table_results.values()]
        )
        for metric in metric_names
    }
    return {
        "summary": summary,
        "per_table": table_results,
        "per_feature": {
            table_name: result.get("per_feature", {})
        ...
        "metadata": {
            ...
            "table_time_cols": {
                table_name: result.get("metadata", {}).get("time_col")
                for table_name, result in table_results.items()
            },
            "table_key_cols": {
                table_name: result.get("metadata", {}).get("key_col")
                for table_name, result in table_results.items()
            },
            "table_max_lags": {
                table_name: result.get("metadata", {}).get("max_lag")
                for table_name, result in table_results.items()
            },
            "requested_max_lag": requested_max_lag,
...
def run_dataset(
    ...
    target_tables = _longitudinal_tables(cfg)
    if not target_tables:
        print(f"[WARN] No config time_col tables found for {dataset}")
        return 0
    ...
    for model in selected_models:
        ...
        if not model_dir.exists():
            ...
            continue
        detected_by_table = {
            table_name: dict(discover_synth_variants(model_dir, table_name))
            for table_name in target_tables
        }
        detected_variants = sorted(
            {
                variant
                for detected in detected_by_table.values()
                for variant in detected
                if not variants or variant in variants
            },
            key=lambda name: list(VARIANT_SUFFIXES).index(name),
        )
        if not detected_variants:
            print(f"[WARN] No synthetic variants for {dataset}/{model}/{target_tables}")
            continue
        for variant in detected_variants:
            ...
            variant_tables = [
                table_name
                for table_name in target_tables
                if variant in detected_by_table[table_name]
            ]
            if dry_run:
                synth_paths = [
                    detected_by_table[table_name][variant].relative_to(ROOT_DIR)
                    for table_name in variant_tables
                ]
                print(
                    f"[DRY RUN] {dataset}/{model}/{variant}: "
                    f"tables={variant_tables} synth={synth_paths} "
            ...
            try:
                ...
                for table_name in variant_tables:
                    ...
                    _, key_col, time_col, real_df, synth_df, num_cols, cat_cols = _prepare_frames(
                        cfg, model_dir, variant, table_name
                    )
                    if not num_cols and not cat_cols:
                        print(
                            f"[WARN] No common feature columns for "
                            f"{dataset}/{model}/{variant}/{table_name}"
                        )
                        continue
                    ...
                    effective_max_lag = (
                        _compute_default_max_lag(real_df, key_col)
                        if max_lag is None
                        else max_lag
                    )
                    table_result = compute_all_longitudinal_metrics(
                        real_df,
                        synth_df,
                        key_col,
                        time_col,
                ...
                if not table_results:
                    print(f"[WARN] No table results for {dataset}/{model}/{variant}")
                    continue
                results = _combine_table_results(
                    table_results,
                    dataset=dataset,
                    model=model,
                    variant=variant,
