...
def flatten_table(
    ...
    for rel in cfg.relationships:
        ...
        if parent_name not in cfg.tables or child_key not in df.columns:
            continue
        parent = flatten_table(cfg, parent_name, real_dir, synth_dir, None, seen)
...
def _add_sibling_aggregates(
    ...
    for sibling_name in cfg.child_tables:
        ...
        if key_col not in sibling_table.columns or time_col not in sibling_table.columns:
            continue
        ...
        agg = sibling.groupby([key_col, time_col], as_index=False)[numeric_cols].sum()
        df = df.merge(agg, on=[key_col, time_col], how="left")
...
def cs__ks_complement(real_grouped, synth_grouped, times, num_col: str) -> float:
    ...
    if no timestamp has sufficient data (at least 2 samples each side).
    ...
    for t in times:
        ...
        if len(real_slice) < 2 or len(synth_slice) < 2:
            continue
...
def cs__statistic_similarity(real_grouped, synth_grouped, times, num_col: str, epsilon: float = 1e-10) -> float:
    ...
    timestamp as ``max(0, 1 - |real_stat - synth_stat| / range_real)``.
    ...
    for stat_func in stat_funcs.values():
        ...
        scores = [
            max(0.0, 1.0 - abs(real_stats[t] - synth_stats[t]) / (range_real + epsilon))
            for t in times
...
def compute_all_cross_sectional_metrics(
    ...
    cont_scores = {
        ...
        for a, b in (contingency_pairs or [])
    }
...
def _contingency_pairs(real_df: pd.DataFrame, cat_cols: list[str], max_cardinality_product: int) -> list[tuple[str, str]]:
    pairs = []
    cardinalities = {col: real_df[col].dropna().astype(str).nunique() for col in cat_cols}
    for a, b in combinations(cat_cols, 2):
        if cardinalities[a] * cardinalities[b] <= max_cardinality_product:
            pairs.append((a, b))
    return pairs
...
def _prepare_dataset_frames(cfg, model_dir: Path, variant: str):
    ...
    real_df, sibling_nums = _add_sibling_aggregates(cfg, real_df, cfg.real_dir, primary, key_col, time_col)
    synth_df, _ = _add_sibling_aggregates(cfg, synth_df, cfg.real_dir, primary, key_col, time_col, model_dir, variant)
...
def run_dataset(
    ...
    dry_run: bool = False,
    max_cardinality_product: int = 80,
) -> int:
    ...
    for model in selected_models:
        ...
        for variant, synth_path in detected:
            ...
            try:
                ...
                _, key_col, time_col, real_df, synth_df, num_cols, cat_cols = _prepare_dataset_frames(
                    cfg, model_dir, variant
                )
                if not num_cols and not cat_cols:
                    print(f"[WARN] No common feature columns for {dataset}/{model}/{variant}")
                    continue
                pairs = _contingency_pairs(real_df, cat_cols, max_cardinality_product)
                ...
                results["metadata"].update(
                    {
                        ...
                        "contingency_pair_count": len(pairs),
                    }
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Maximum cardinality product for categorical contingency pairs.",
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
...
def main() -> int:
    ...
    def _run_target(dataset: str) -> int:
        ...
        return run_dataset(
            ...
            max_cardinality_product=args.max_cardinality_product,
        )
