...
METRIC_ALIASES = {
    "all": ["structural", "sd", "timestamp", "cross_sectional", "longitudinal", "trajectory_privacy"],
    "cs": ["cross_sectional"],
    "cross-sectional": ["cross_sectional"],
    "cross_sectional": ["cross_sectional"],
    "long": ["longitudinal"],
    "longitudinal": ["longitudinal"],
    "privacy": ["trajectory_privacy"],
    "trajectory-privacy": ["trajectory_privacy"],
...
class Seq2Synth:
    ...
    def build_runs(self) -> list[Seq2SynthRun]:
        ...
        for metric in self.metrics:
            ...
            elif metric == "trajectory_privacy":
                runs.extend(self._trajectory_privacy_runs())
            else:
    ...
    def _trajectory_privacy_runs(self) -> list[Seq2SynthRun]:
        runs: list[Seq2SynthRun] = []
        for dataset in self._filter_supported(
            "trajectory_privacy",
            self.datasets,
            ...
            cmd = [
                sys.executable,
                str(Path("seq2synth") / "metrics" / "trajectory_privacy" / "run_trajectory_privacy.py"),
                "--dataset",
                dataset,
                "--model",
                *models,
            ...
            if self.workers is not None:
                cmd.extend(["--workers", str(self.workers)])
                cmd.extend(["--n_jobs", str(self.workers)])
            runs.append(Seq2SynthRun(dataset, None, "trajectory_privacy", cmd))
        return runs
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["all"],
        help="Metrics to run: all, structural, sd, timestamp, cross_sectional, longitudinal, trajectory_privacy.",
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
